...
abstract contract Views is Storage {
    function collateralConfig(address user, address token)
        public
        view
        returns (CollateralConfig memory)
    {
        ...
        return CollateralConfig({
            priceFeed: feed, customCollateralRatio: ratio, customLiqThreshold: liqThreshold
        });
    }
    ...
    function positions(address user, address token) public view returns (PositionConfig memory) {
        ...
        return PositionConfig({
            collateralDeposited: totalCollateralDeposited, mintedNgns: totalNgnsDebt
        });
    }
